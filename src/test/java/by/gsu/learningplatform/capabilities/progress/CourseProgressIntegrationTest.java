...
class CourseProgressIntegrationTest extends EndpointIntegrationTestSupport {
    ...
    void setUp() {
        ...
        val lessonId = insertLesson(courseId, "Progress lesson");
    ...
    void shouldAllowStudentToTrackOwnProgress() throws Exception {
        val update = put(progressLecturePath(studentId, lectureId), "{\"completed\":true}", studentToken);
        ...
        val read = get("/courses/" + courseId + "/progress/me", studentToken);
    ...
    void shouldAllowCourseTeacherToReadStudentProgress() {
        val studentUpdate = put(progressLecturePath(studentId, lectureId), "{\"completed\":true}", studentToken);
        assertEquals(200, studentUpdate.statusCode());

        val read = get("/courses/" + courseId + "/users/" + studentId + "/progress", teacherToken);
        assertEquals(200, read.statusCode());
    }

    @Test
    void shouldForbidCourseTeacherWhenUpdatingStudentProgress() {
        val update = put(progressLecturePath(studentId, lectureId), "{\"completed\":true}", teacherToken);
        assertEquals(403, update.statusCode());
    }

    @Test
    void shouldForbidNonOwnerTeacher() {
        val response = get("/courses/" + courseId + "/users/" + studentId + "/progress", token("other-teacher-token"));
        assertEquals(403, response.statusCode());
    }

    @Test
    void shouldRejectLectureFromAnotherCourse() {
        val otherCourseId = insertCourse(otherTeacherId, "Other course");
        val otherLessonId = insertLesson(otherCourseId, "Other lesson");
        val otherLectureId = insertLecture(otherLessonId, "Other lecture");

        val response = put(progressLecturePath(studentId, otherLectureId), "{\"completed\":true}", studentToken);
        assertEquals(400, response.statusCode());
    }

    private String progressLecturePath(final UUID userId, final UUID targetLectureId) {
        return "/courses/" + courseId + "/users/" + userId + "/progress/lectures/" + targetLectureId;
    }

    private String token(final String value) {
        return value;
    }
}
